...
test("upgrade regenerates derived artifacts, so a config fix actually reaches a workspace", () => {
  const ws = makeWorkspace();
  const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "awo-upgvs-")));
  fs.writeFileSync(path.join(repo, "README.md"), "# x\n");
  execFileSync(process.execPath, [CLI, "connect", repo, "--name", "api"], { cwd: ws });

  // Simulate a workspace generated by an older version: settings in the wrong place.
  const file = path.join(ws, "UP.code-workspace");
  const cfg = JSON.parse(fs.readFileSync(file, "utf8")) as Record<string, unknown>;
  fs.writeFileSync(file, JSON.stringify({ folders: cfg.folders, settings: {} }, null, 2));
  pretendOlder(ws, "0.0.37");
