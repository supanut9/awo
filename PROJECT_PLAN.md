...
4. **Frozen at init.** A workspace does not auto-pull library updates. But the manifest records `libraryVersion`, costing nothing now and leaving the door open for an `upgrade` command later.
...
| `awo sync` | Reconcile `repos/` with the manifest (clone/pull git, relink local). |
...
| Command | Purpose |
|---------|---------|
| `awo req new` | Intake: gather + refine a requirement. |
| `awo goal new --from <req-id>` | Transform a requirement into a goal. |
