...
import { TASK_STATUSES } from "./state.js";
/**
...
async function goalDirs(workspaceRoot) {
...
export async function readTaskFile(file) {
    ...
    if (!TASK_STATUSES.includes(authored)) {
        throw new Error(`${file} has status "${raw}", which is not a task lifecycle state. Valid: ${TASK_STATUSES.join(", ")}.`);
    }
    return {
        id: fm.id.trim(),
...
export async function findGoals(workspaceRoot) {
    const goals = [];
    for (const dir of await goalDirs(workspaceRoot)) {
        const goalFile = path.join(dir, "goal.md");
        if (!(await fs.pathExists(goalFile)))
            continue;
        const fm = matter(await fs.readFile(goalFile, "utf8")).data;
...
export async function findTasksInGoal(goalDir) {
    ...
    for (const file of files)
        tasks.push(await readTaskFile(file));
    return tasks;
}
export async function findAllTasks(workspaceRoot) {
    const located = [];
...
export async function locateTask(workspaceRoot, taskId) {
